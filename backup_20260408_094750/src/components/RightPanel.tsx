...
export const RightPanel = memo(() => {
  ...
  const autoExpertMode = useStore(state => state.autoExpertMode);
  const setAutoExpertMode = useStore(state => state.setAutoExpertMode);
  const selectedExpertIds = useStore(state => state.selectedExpertIds);
  const toggleExpertSelection = useStore(state => state.toggleExpertSelection);
  const isGenerating = useStore(state => state.isGenerating);
  const setIsGenerating = useStore(state => state.setIsGenerating);
  const addNode = useStore(state => state.addNode);
  const currentMode = useStore(state => state.currentMode);
  const generationTurn = useStore(state => state.generationTurn);
  const setGenerationTurn = useStore(state => state.setGenerationTurn);
  ...
  // nodes 전체 구독 제거: 선택된 노드만 직접 셀렉터로 추출
  // → 드래그 중 다른 노드 위치가 바뀌어도 RightPanel은 재렌더되지 않음
  const selectedNode = useStore(state => {
    const id = state.selectedNodeId?.includes('::')
      ? state.selectedNodeId.split('::')[0]
      : state.selectedNodeId;
    return id ? state.nodes.find(n => n.id === id) : undefined;
  });

  // 다중 선택 노드들 — useShallow로 배열 내용 비교 (새 참조 생성에 의한 무한 루프 방지)
                ...
                <div className="p-5 pt-0 bg-[#fcfcfc]">
                  <button
                    ...
                    disabled={
                      ...
                      (selectedNodeIds.length > 1 && selectedNodes.every(n => {
                        if (!n) return true;
